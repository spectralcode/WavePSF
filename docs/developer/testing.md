...
From the repository root, in an MSVC shell with Qt's `bin` on `PATH` (for
`qmake`, and so the test exes find their DLLs):
...
5. For a regression whose failure mode is a hang, start a watchdog thread that
