	...
	const DeconvolutionCancelToken* cancelToken)
{
	...
	LOG_INFO() << "Batch 2D deconvolution finished:" << result.completedUnits << "jobs processed,"
			   << result.failedUnits << "failed";
	return result;
}
	...
	const DeconvolutionCancelToken* cancelToken)
{
	...
	LOG_INFO() << (singlePatch3D ? "Single-patch" : "Batch")
			   << "3D deconvolution finished:" << result.completedUnits << "jobs processed,"
			   << result.failedUnits << "failed";
	return result;
}
